...
import { FiBarChart2, FiCalendar, FiCheck, FiChevronDown, FiDollarSign, FiDownload, FiFileText, FiFilter } from 'react-icons/fi';
...
export default function ReportsPage() {
  ...
  const fetchReport = () => {
    // This is now handled by SWR automatically
  };
                  ...
                  <div className="card p-5">
                    ...
                    {reportData.categoryBreakdown?.length > 0 ? (
                      <div className="space-y-2">
                        {reportData.categoryBreakdown.map((c: { type: string; category: string; total: number; count: number }, i: number) => (
                          <div key={i} className="flex items-center justify-between py-2 border-b border-surface-50 last:border-0">
                            <div className="flex items-center gap-2">
                              <div className={`w-2 h-2 rounded-full ${c.type === 'income' ? 'bg-green-500' : 'bg-red-400'}`} />
                              <span className="text-sm text-surface-700">{c.category}</span>
                              <span className="text-xs text-surface-400">({c.count})</span>
                            </div>
                            <span className={`text-sm font-semibold ${c.type === 'income' ? 'text-green-600' : 'text-red-500'}`}>
                              {formatBDT(c.total)}
                            </span>
                          </div>
                        ))}
          ...
          <div className="space-y-6">
            <div className="card p-4">
              <button onClick={fetchReport} className="btn-primary">
                <FiDollarSign size={14} /> Calculate All Cow Costs
              </button>
            </div>
              ...
              <div className="table-container">
                <div className="overflow-x-auto">
                  <table>
                    ...
                    <tbody>
                      {reportData.cows.map((cow: any) => (
                        <tr key={cow._id}>
                          <td className="font-medium">{cow.tag}</td>
                          <td>{cow.name || '—'}</td>
                            ...
                            <div key={i}>
                              <div className="flex justify-between text-sm mb-1">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-surface-700">{c.category}</span>
                                  <span className="text-xs text-surface-400">({c.count})</span>
                                </div>
                                <span className={`font-semibold text-${color}-600`}>{formatBDT(c.total)}</span>
                              </div>
